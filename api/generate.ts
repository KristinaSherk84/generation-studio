...
const BLOCK_6_BACKGROUND: Record<Exclude<Background, "rainbow">, string> = {
  ...
  bluebright: `Background: Highly saturated Klein blue with slight gradient to deeper Klein blue at the background edges. Place a spot light on the background illuminating the circle directly behind the subject's lower neck and upper shoulders. The brightest part of the spot light should be bright "dell logo" blue with a very gradual gradient transitioning outward in a circle. Add soft rim lights to the subject on either side to separate them from the background.`,
