...
export default async function handler(
  ...
  try {
    ...
    if (similarImage) {
      ...
      const wideSuffix = commonPreamble + `\n3. EXPRESSION: match the target reference's expression closely.\n4. FRAMING (CRITICAL — this variation exists specifically to give the customer more breathing room): apply a SIGNIFICANTLY WIDER crop than the target — pull the camera back by roughly 30–40% so the frame includes the entire upper torso down to at least mid-chest, and a comfortably wider view of the background. The subject's head should occupy roughly HALF the frame width in the output (versus filling most of the target's width). Wider only — never crop tighter than the target. If in doubt, err wider.\n5. VARIATION: subtle only, other than the significantly wider crop.`;
