...
const BLOCK_UNDER_EYE = `Under-eye rule (women only). Use reference photos' under-eye condition as a base reference, improve under-eye wrinkle appearance and brighten under eye skin. Render bright, well-rested, under-eyes with native skin texture.`;
...
const BLOCK_3_STYLE_BASE: Record<Style, string> = {
  ...
  urban: `Style: Modern, on-location, lifestyle. Reads as a polished professional photographed in a real city environment — a downtown senior tech leader, a tech founder, a content creator with executive presence. Far away city street background. Background is always a real urban setting (city street, industrial space) rendered with super creamy, extreme bokeh (shot with a 200mm lens at f/1.2) so no specifics are identifiable. Facial expression must be realistic, spunky, fresh, active, engaged.`,
