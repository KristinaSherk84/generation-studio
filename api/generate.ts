...
const BLOCK_UNDER_EYE = `Under-eye rule (women only). Use reference photos' under-eye condition as a reference but improve under-eye skin appearance. Render bright, well-rested, realistic, bright under-eyes with native skin texture.`;
...
Include a subtle fill from below the subject to soften — not eliminate — the under-eye, under-nose, and under-chin shadows. Keep medium shadows present for a shaped, directional look; no flattened lighting. Hair rim light preserves separation. Background falls to near-black. Both eyes must always carry at least one visible catchlight.`,
  golden: `Lighting: Warm, low-angled light as if from a late-afternoon sun. Hair rim light from behind. Strong bounced, warm fill light on the shadow sides from multiple gold reflectors. Add a strong, soft fill from a low frontal position that fully lifts and opens the eye area: completely fill the under-eye area and the orbital sockets so there are no dark circles, no hollow or shadowed eye sockets, and no raccoon-eye shading — the eyes read bright, open, and well-rested. Illuminate all downward-facing planes of the face (under the brow, under the nose, under the chin). Warm color grade, but skin tones stay true — no orange cast.`,
};
