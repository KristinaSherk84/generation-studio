...
SKIN RENDERING: Render extremely realistic skin texture, exactly as it appears in the reference photos. Remove any temporary blemishes (pimples, acne, spots, breakouts) that ARE present in the reference photos; do NOT invent, add, or hallucinate any blemish, pimple, spot, or breakout that is not clearly visible in the references. Keep pores, freckles, moles, and texture EXACTLY as visible in the references. Render authentically real skin — no airbrushed look. 

PROTECT and PRESERVE unique skin marks: Render ALL skin marks visible in reference photos. DO NOT CREATE or ADD ADDITIONAL blemishes, pimples, acne, spots, breakouts, bumps, freckles, moles, beauty marks, scars, or age spots that are not seen in reference photos.
...
const BLOCK_3_STYLE_BASE: Record<Style, string> = {
  corporate: `Style: Clean studio-style corporate headshot. Subtle confidence, approachable but professional — director-level expression and energy. Photography background matches the color specified below at 95% fidelity. Add extreme confidence through the eyes expressions. The eyes must be realistic, activated, engaged, and smiling. As if the eyes have a secret they are keeping.`,
  creative: `Style: Warm, approachable, personable, with a clear outdoor background. Softer edges than corporate. FRAMING: a tight head-and-shoulders headshot — the subject is CLOSE to the camera and fills the frame exactly like a studio headshot, with the top of the head near the top of the frame and minimal headroom above it; only the BACKGROUND is far away. Do NOT pull the camera back, do NOT shrink the subject within a wide outdoor scene, do NOT show the full body. The natural setting (trees, garden, foliage) sits far BEHIND the subject, rendered with extreme creamy bokeh. Expressions full of competent, confident personality. Add confidence through the eyes expressions. The eyes must be realistic, caring, etherial, and confidently smiling, as if they know all the secrets.`,
  ...
  urban: `Style: Modern, on-location, lifestyle. Reads as a polished professional photographed in a real city environment — a downtown senior tech leader, a tech founder, a content creator with executive presence. FRAMING: a tight head-and-shoulders headshot — the subject is CLOSE to the camera and fills the frame exactly like a studio headshot; only the BACKGROUND is far away. Do NOT pull the camera back, do NOT shrink the subject within a wide street scene, do NOT show the full body. The real urban setting (city street, industrial space) sits far BEHIND the subject, rendered with super creamy, extreme bokeh (as if shot on a 200mm lens at f/1.2) so no specifics are identifiable. Facial expression must be realistic, spunky, fresh, active, engaged.`,
...
const URBAN_BG_INDUSTRIAL = `Background: extremely blurry, bright, modern industrial office interior — exposed concrete, steel beams, large windows flooded with light. Apply DOUBLE the background blur — photographed with the most extreme creamy bokeh imaginable (as if shot on a 300mm lens at f/1.0 with the background 120+ feet behind the subject). The background must be SO heavily blurred that NO specific beam, window, wall, surface, edge, or object is identifiable — no straight architectural lines should survive. What should be visible: large creamy bokeh orbs and soft ambient light, abstract geometric washes in light grey, white, and cool tones. What must NOT be visible: any recognizable architectural detail, visible beam, window frame, corridor line, or piece of furniture. Think impressionist painting of light — an abstract wash of tone, NOT a photo of an office.`;

const URBAN_BG_STREET = `Background: Extremely blurry city sidewalk and storefronts at golden hour — brick facades, shop windows, awnings, railings, the subtle suggestion of distant pedestrians. Apply DOUBLE the background blur — architectural elements placed 120+ feet behind the subject, photographed with the most extreme creamy bokeh imaginable (as if shot on a 300mm lens at f/1.0). The background must be SO heavily blurred that NO specific sign, doorway, window mullion, railing, person, or business is identifiable — no straight architectural lines or pedestrian shapes should survive. What should be visible: warm afternoon golden-hour light, large creamy bokeh orbs, soft architectural color washes (warm brick reds, deep stone greys, warm window glows), gentle out-of-focus highlights. What must NOT be visible: any recognizable storefront, sign text, doorway, pedestrian, vehicle, or hard edge. Think impressionist painting of a warm city street — a soft wash of warm tones, NOT a recognizable place.`;
...
const BLOCK_5_LIGHTING: Record<Lighting, string> = {
  studio: `Lighting: Broad, super soft key light placed slightly above of the subject. A substantial fill light source from below illuminating the shadows under the brow bones, the nose. Fill is barely darker than the key light. Aggressive under-eye / under-nose fill light from a low frontal position — COMPLETELY ELIMINATE shadows in the eye sockets, under the nose, in the smile-line creases. Skin under those features must read as evenly lit as the rest of the face — no shadow at all in those zones, only smooth even illumination. Two or three catchlights in the eyes from the key, fill, and under-eye fill sources. THE ONLY PERMITTED SHADOW on the entire face/neck region is a soft, narrow gradient under the jawline (lower jaw + upper neck) — preserve that to give the subject jaw definition and separation from the body.`,
  natural: `Lighting: Large window light as key, angled at roughly 25 degrees to the subject. Warm 4000K–5000K color temperature. Other face areas lit with warm bounced fill lights from multiple angles — warm, golden, reflective surfaces bounce light back to the face from many angles, a large bounce below the subject. COMPLETELY ELIMINATE shadows in the eye sockets, under the nose, in the smile-line creases. Skin under those features must read as evenly lit as the rest of the face — no shadow at all in those zones, only smooth even illumination. Add catchlights in the bottoms of the eyes. Organic and slightly directional, no deep shadows.`,
...
Include a subtle fill from below the subject to soften — not eliminate — the under-eye, under-nose, and under-chin shadows. Keep medium shadows present for a shaped, directional look; no flattened lighting. Hair rim light preserves separation. Background falls to near-black. Both eyes must always carry at least one visible catchlight.`,
  golden: `Lighting: Warm, low-angled light as if from a late-afternoon sun. Hair rim light from behind. Large warm 5 foot soft box as key light, angled at roughly 25 degrees to the subject, and slightly above. Warm 4000K–5000K color temperature. Other face areas lit with warm bounced fill lights from multiple angles — warm, golden, reflective surfaces bounce light back to the face from many angles, a large bounce below the subject. COMPLETELY ELIMINATE shadows in the eye sockets, under the nose, in the smile-line creases. Skin under those features must read as evenly lit as the rest of the face — no shadow at all in those zones, only smooth even illumination. Add catchlights in the bottoms of the eyes. Organic and slightly directional, no deep shadows.`,
};
...
const BLOCK_6_BACKGROUND: Record<Exclude<Background, "rainbow">, string> = {
  ...
  blue: `Background: Dark saturated navy with slight gradient to deeper black at edges. Place a spot light on the background illuminating the circle directly behind the subject's lower neck and upper shoulders. The brightest part of the spot light should be vivid royal blue with a very gradual gradient transitioning outward. Add very soft rim lights to the subject. Add cool tonality to the whole person in the photo to match the coloring to the background; avoid generating a person with too much warm light. Especially add cool color tonality to the shadow areas of the subject.`,
  // Bright Blue (2026-08-06 handoff) — Klein blue base with a bright "Dell
  // logo" blue spotlight; modern/tech feel vs. the enterprise navy of "blue".
  // Prompt stored VERBATIM per Kristi (tested phrasing — do not reword).
  bluebright: `Background: Highly saturated Klein blue with slight gradient to deeper Klein blue at the background edges. Place a spot light on the background illuminating the circle directly behind the subject's lower neck and upper shoulders. The brightest part of the spot light should be bright "dell logo" blue with a very gradual gradient transitioning outward in a circle. Add soft rim lights to the subject on either side to separate them from the background. Add cool tonality to the whole person in the photo to match the coloring to the background; avoid generating a person with too much warm light. Especially add cool color tonality to the shadow areas of the subject.`,
...
function buildBlock7Technical(
  ...
  return stripDof(`Technical quality: 2048-pixel resolution, sharp focus on the eyes, eyelashes visible. Skin: Preserve the 3D pore micro-texture (the raised/recessed surface terrain at close magnification) per Block 1's directive. Realistic, clear skin. Do NOT create plastic, filter-smoothed skin. Very shallow depth of field — subject's face in perfect focus, shoulders softly falling off, background noticeably blurred. Professional color grading: accurate skin tones, no color cast, slight warmth in shadows. No visible artifacts, no uncanny valley. This is a commercial-grade photograph, extremely realistic — not an illustration, render, or composite. Skin redness in reference photos can be removed.`);
}
...
const BLOCK_LENS_CORRECTION = `CRITICAL LENS CORRECTION: If selfie camera was used to take reference photos, and CONFIRMED via EXIF metadata to be shot with a wide-angle lens (35mm-equivalent focal length under 40mm — typically a phone selfie camera at 24–28mm equivalent). FULLY CORRECT any lens distortion in the generated headshot. Render the subject's face as if photographed with a prime 135mm portrait lens: the nose sits in correct proportion to the face, no bulging nose, no stretched forehead, no "selfie face." This correction is MANDATORY.`;
...
const BLOCK_EYEWEAR = `Eyewear: Only if reference photos have glasses, preserve the same glasses in the generated headshot — match the frame shape, frame color, and material as closely as possible. Remove all tint and all reflections from lenses. If the subject is NOT wearing glasses in the reference photos, do NOT add glasses.`;
...
export const PROMPT_DEFAULTS: Record<string, string> = {
  ...
  slot_pose_0: FLAVORS[0].bodyPose,
  slot_crop_0: FLAVORS[0].crop,
  slot_pose_1: FLAVORS[1].bodyPose,
  slot_crop_1: FLAVORS[1].crop,
  slot_pose_2: FLAVORS[2].bodyPose,
  slot_crop_2: FLAVORS[2].crop,
  slot_pose_3: FLAVORS[3].bodyPose,
  slot_crop_3: FLAVORS[3].crop,
  slot_pose_4: FLAVORS[4].bodyPose,
...
export const PROMPT_SEGMENTS: PromptSegmentMeta[] = [
  ...
  { key: "style_tech", label: "Style — IT/Tech", group: "Style", fires: { style: "tech" } },
  { key: "tech_bg_serverroom", label: "IT/Tech — server-room background", group: "Style", fires: { style: "tech" } },
  ...
  { key: "slot_pose_0", label: "Slot 1 — Body angle", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_crop_0", label: "Slot 1 — Framing / crop", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_pose_1", label: "Slot 2 — Body angle", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_crop_1", label: "Slot 2 — Framing / crop", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_pose_2", label: "Slot 3 — Body angle", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_crop_2", label: "Slot 3 — Framing / crop", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_pose_3", label: "Slot 4 — Body angle", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_crop_3", label: "Slot 4 — Framing / crop", group: "Per-slot pose & framing", fires: {} },
  { key: "slot_pose_4", label: "Slot 5 — Body angle", group: "Per-slot pose & framing", fires: {} },
...
export default async function handler(
  ...
  if (
    ...
    body.background &&
    !["white", "lightgrey", "dark", "black", "blue", "bluebright", "green", "red", "rainbow"].includes(body.background)
  ) {
  ...
  try {
    ...
    if (similarImage) {
      ...
      const widePreamble = `\n\nGENERATE VERSIONS OVERRIDE (2026-09-04 · WIDE VARIANT): The FIRST image is a REFERENCE for OUTFIT + HAIR + BACKGROUND + LIGHTING only — NOT for framing or crop. This variant deliberately produces a WIDER shot of the same person in the same outfit and setting.\n\nALWAYS refer to the identity reference photos (images 2+, uploaded by the customer) for THIS PERSON's actual identity — their face shape, jawline, skin tone, hair texture, eyes, ears, natural proportions. Cross-check every feature against those reference photos as you generate — they are the ground truth for who this person is.\n\n1. COMPOSITION / FRAMING (this variation exists SPECIFICALLY to give the customer a wider version — this is the ONE thing you must change from the reference): compose a new professional headshot at HEAD-AND-SHOULDERS-AND-CHEST framing. The frame includes the subject's full head with generous space around it, both full shoulders visible edge-to-edge, and the entire upper chest down to below the sternum. The subject's head occupies roughly ONE-THIRD to ONE-HALF of the frame width — think classic corporate headshot proportions, NOT a tight face crop. The output is noticeably WIDER, TALLER, and PULLED BACK relative to the reference. Absolutely do not crop the same as the reference; the reference is a close-up and this variant is a MEDIUM shot.\n2. IDENTITY: comes entirely from the identity reference photos (images 2+). Preserve their face with high fidelity — the SAME person, unmistakably. The identity reference photos also naturally show the person at wider distances than the target reference — use them to see how the subject's shoulders, neck, and upper body actually look at the wider framing this variant needs.\n3. OUTFIT: match the outfit in the reference (same garment, same color).\n4. HAIR: match the hairstyle in the reference.\n5. BACKGROUND: same setting as the reference, but visible in a wider view (more of the background is in frame because the camera has pulled back).\n6. LIGHTING: match the reference's lighting direction, quality, and warmth.\n7. EXPRESSION: match the reference's expression closely — this variant is about FRAMING, not expression.`;
      const wideSuffix = widePreamble + `\n\nFINAL COMPOSITION CHECK — the output MUST be a wider medium shot than the reference. If your first draft looks similar in crop to the reference, redo it wider before returning it. Head-and-chest, not close-up. This is the entire point of this variant.`;
