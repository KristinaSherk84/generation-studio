...
// Glam — all ages. v7 2026-05-18: confirmed locked by Kristi after
// multi-subject testing in retouch-prompt-tester.html. Key changes
// from v3:
//   - SKIN AROUND THE EYES gained "Soften on all images, regardless
//     of current state" — overrides the model's tendency to apply
//     less retouching when the input photo already reads as polished
//   - MAKEUP gained explicit "soft, thin, powder eye liner" + brow fill
//   - ADD CONTOUR replaces "bronzing" with "darkening" (avoids
//     unintended warmth/orange shift)
//   - Master directive references "beauty skin campaign skin" /
//     Allure / Estée Lauder commercial campaign
//   - SURFACE EVENNESS explicitly targets neck horizontal lines
//   - PORE STRUCTURE allows wrinkles to be "almost completely removed"
//   - Outer-corner crow's feet "shorten by half" calibration
export const RETOUCH_GLAM = `${IDENTITY_ANCHOR}

GLAM RETOUCH (editorial luxury beauty):

Master directive: Editorial luxury beauty retouching, equivalent to beauty skin campaign skin. Or an Allure beauty cover, or an Estée Lauder commercial campaign. Skin should look like a commercial-beauty-campaign examples. Skin retains pore detail and structure; tone renders flawlessly even and illuminated. The skin around the eyes renders editorial-flawless — luminous with even tone, like a magazine beauty shot. Filled-in softbox lighting with almost no shadows, professionally retouched in post-production by a high-end beauty retoucher. Skin reads as commercial-beauty-campaign-quality but still retains all pore structure. CRITICAL IDENTITY GUARDRAIL: at editorial-level smoothing the model has a strong tendency to drift toward generic-pretty / AI-default features and lose the subject's actual identity — do NOT let that happen. The smoothing only applies to surface evenness. Every facial feature, every proportion, every distinguishing mark, the eye SHAPE itself, the nose, the mouth, the bone structure, the asymmetries — all of those remain UNMISTAKABLY the subject's own. Smooth the surface, not the person.

The aesthetic target is "commercial beauty campaign that hasn't erased the human" — Lancôme/Allure-level retouching where the model is dramatically polished but still has visible pores under close inspection. Polished, even-toned, glowing, aspirational.

- MOUTH AND SMILE LOCK (CRITICAL — OVERRIDES EVERYTHING BELOW): The mouth, smile, lips, and teeth in the output MUST be IDENTICAL to the input photo. This is non-negotiable. If the input shows a closed-mouth smile, the output shows the SAME closed-mouth smile — do not open the mouth, do not show teeth, do not change the expression. If the input shows teeth, the output shows the SAME teeth in the SAME arrangement — do not change which teeth are visible, do not change the smile width, do not change the smile shape. Lip shape, lip thickness, lip volume, lip outline, mouth corners, smile asymmetry — ALL must match the input photo exactly. The retouching operates on skin SURFACE only — the underlying mouth, smile, and expression remain UNTOUCHED. If the output shows a different smile than the input, even slightly, you have failed this directive.
