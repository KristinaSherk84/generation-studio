...
// Source list keyed by source-pair index 0..42. Each entry references the
// actual file on disk (filename has both template slug + index baked in).
// Indices 0-31 = original set; 32-42 = 11 pairs added 2026-08-20 (built from
// Before-After Graphics pairs 56-66). Indices 16/17/18 still generate an entry
// here but are filtered out of the display order below (customer removal —
// see GALLERY_REMOVED); their files remain on disk, just unreferenced.
...
// Source indices pulled from the gallery at the customer's request
// (2026-08-20): one model whose three outfit variants were source indices
// 16, 17, and 18. Filtered out of the display order below; their image files
// still exist on disk but are never referenced.
const GALLERY_REMOVED = new Set<number>([16, 17, 18]);
...
// so React keys are stable across renders. The original 0-31 set is
// interleaved (minus the removed indices); the 11 pairs added 2026-08-20
// (indices 32-42) are appended after.
const SHUFFLE_ORDER: number[] = (() => {
  ...
  const kept = arr.filter((i) => !GALLERY_REMOVED.has(i));
  for (let i = 32; i <= 42; i++) kept.push(i);
  return kept;
})();
...
// -------------------- Gallery Screen --------------------
//
// Full-screen gallery of all before/after pairs (40 after the 2026-08-20
// add/remove). Accessible from the
...
const GalleryScreen = ({ onBack, onStart, entryFeeEnabled }: GalleryScreenProps) => (
  ...
  >
    ...
    >
      ...
      >
        40 transformations from selfie to professional headshot. Each one
...
const LandingV2 = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* 2026-09-03: H1 rewritten from "AI Headshot Generator" to
            "AI headshots made by a real photographer" per Kristi. Restores
            the photographer-provenance angle as the lead message — the
            primary keyword is still in the H1, and the differentiator no
            longer needs to fight for space in the subhead. */}
        <h1
          style={{
            fontFamily: SERIF_STACK,
            fontSize: isMobile
...
type LoadingScreenProps = {
  ...
  // Re-fire the same batch from the "Connection interrupted" popup without
  // sending the user back to the style screen (2026-08-01).
  onRetry: () => void;
};
...
const LoadingScreen = ({
  ...
  return (
    ...
    >
      ...
      {/* "Connection interrupted" popup (2026-08-01). A zero-success batch is
          almost always the client losing its connection mid-generation (tab
          closed / backgrounded / navigated away) — the server logs show the
          images generating fine in these cases. So instead of the old
          photo-blaming copy we surface an honest modal that tells the user to
          stay on the page and offers a one-tap retry. Keyed on the message
          prefix so other (pre-generation) errors keep the plain inline
          treatment above. */}
      {errorMessage && errorMessage.startsWith("Connection interrupted") && (
        <div
          role="dialog"
          aria-modal="true"
          style={{
        ...
        >
          ...
          >
            ...
            >
              The connection to this page dropped while your headshots were
              being created. This usually happens when the tab is closed, put
              in the background, or you navigate away during the 2–3 minutes it
              takes. Your uploaded photos are fine — please stay on this page
              and we&rsquo;ll try again.
            </p>
...
const GridScreen = ({
  ...
  // Extras collapse state (2026-09-03). When a customer generates more than
  // one batch, the accumulated session can hold 7+ shots (batch 1 + batch 2 +
  // regens that got appended). The main grid only renders the first 6 slots,
  // so anything past slot 5 was invisible — customers only saw their earliest
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "64px 32px", ...font }}>
      ...
      {/* EXTRAS — additional headshots from earlier batches in the same
          session (2026-09-03, per Kristi). When a customer generates more
          than one batch, save-session accumulates every batch's shots + any
          appended regens under ONE resume token, so state can hold 7+ URLs.
          The main grid only paints slots 0-5, which means shots past index 5
          were invisible even though the server had them — customer clicks
          RTV email link, only sees the first 6. Fix: show any extras here in
          a collapsed pill ("Expand to see N more headshots you generated
          today"). Tap to reveal — same +cart + lightbox mechanics as the
          ...
          {extrasExpanded && (
            ...
            >
              {images.slice(6).map((src, idx) => {
                ...
                const isFailed = failedUrls.has(src);
                return (
                  <div
                    key={globalIndex}
                    onClick={() => {
                      if (!src || isFailed) return;
                      if (picked) onRemoveFromCart(src);
                      else if (!cartIsFull) onAddToCart(src);
                    }}
                    style={{
                      position: "relative",
                      ...
                      borderRadius: 8,
                      overflow: "hidden",
                      cursor: src && !isFailed ? "pointer" : "default",
                      border: `2px solid ${picked ? C.dark : "transparent"}`,
                      transition: "border-color 0.15s",
                    }}
                  >
                    {!loadedUrls.has(src) && !isFailed && (
                      <div
                        style={{
                          position: "absolute",
                          inset: 0,
                    ...
                    />
                    {/* Watermark overlay (same anti-theft pattern as main grid) */}
                    <div
                      style={{
                        position: "absolute",
                        inset: 0,
                    ...
                    {/* Add / remove from cart badge (top-right) */}
                    <div
                      style={{
                        position: "absolute",
                        top: 8,
                    ...
                    {/* Expand / lightbox button (bottom-left). Reuses the same
                        preview state as the main grid tiles. previewIndex is
                        set to null so the lightbox's left/right arrows don't
                        try to cycle over the main-grid array — extras open as
...
const FOUND_VIA_OPTIONS = [
  // Order set by Kristi 2026-08-10: "Best Generator" Article first; the
  // 4th slot (where reported non-closing taps cluster) is deliberately a
  // less-advertised source so a stray mis-tap there costs the least.
  // 2026-09-04: swapped Facebook → Reddit per Kristi (new channel focus).
...
// -------------------- Before/after slider --------------------
// Small self-contained draggable comparison slider (2026-09-03). Used inside
// LoadingRetouchPreviewModal to let customers drag between Realistic and
// Polished / Realistic and Glam to see the retouching difference. Works with
// both mouse (drag anywhere on the image) and touch (drag anywhere on the
// image on mobile). Keyboard arrow keys move the divider when focused.
type RetouchBeforeAfterSliderProps = {
  beforeSrc: string;
  afterSrc: string;
  beforeLabel: string;
  afterLabel: string;
...
const RetouchBeforeAfterSlider = ({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="slider"
      aria-label={`Drag to compare ${beforeLabel} to ${afterLabel}`}
      ...
      style={{
        position: "relative",
        width: "100%",
        // Source images are 515×676 (cropped from the original 515×967 by
        // trimming the bottom 30% on 2026-09-03 to shorten the slider so the
        // whole popup fits an iPhone SE screen at 240px cap without needing
    ...
    >
      ...
      />
      {/* AFTER image (top layer, clipped by divider position) */}
      <div
        style={{
          position: "absolute",
          inset: 0,
...
// Tab wrapper for the before/after slider — two tabs (Realistic vs Glam
// default, Realistic vs Polished secondary). Swapping tabs remounts the
// underlying slider so the divider resets to 50% and the image cross-fade
// is instant. (2026-09-03)
const RetouchTabbedSlider = () => {
  const [tab, setTab] = useState<"glam" | "polished">("glam");
  const tabBtn = (value: "glam" | "polished", label: string) => {
  ...
  return (
    <div>
      ...
      >
        {tabBtn("polished", "Polished")}
        {tabBtn("glam", "Glam")}
      </div>
      {tab === "glam" ? (
        <RetouchBeforeAfterSlider
          key="glam"
          beforeSrc="/marketing/retouch-realistic.jpg"
      ...
      {/* Caption below the slider removed 2026-09-03 per Kristi — the ⇔
          handle + labels on the corners are self-explanatory, and dropping
          the caption tightens the popup further. */}
    </div>
  );
};
...
const LoadingRetouchPreviewModal = ({
  ...
  // 2026-09-03: the card stops propagation and the backdrop only fires when
  // the click target IS the backdrop itself, so slider drags / tab switches
  // inside the card never trigger dismiss.
  ...
  <div
    ...
    // Backdrop click-to-dismiss (2026-08-19). ONLY dismiss when the click
    // target IS the backdrop itself, not something inside the card. Before
    // (2026-09-03) this fired on any bubbled click, so tapping the new
    // interactive slider handle used to close the modal — the slider's
  ...
  >
    <div
      // Absorb click bubbling so ANY interaction inside the card (slider
      // drags, tab switches, tap-to-focus) never triggers the backdrop
      // dismiss above. Defense in depth on top of the target-guard on the
      // backdrop — either alone would work; both together are unambiguous.
      onClick={(e) => e.stopPropagation()}
      style={{
        position: "relative",
        background: C.white,
    ...
    >
      ...
      {/* Interactive before/after slider (2026-09-03 v3, per Kristi).
          ...
          Glam (default, most dramatic) and Realistic vs Polished (subtler).
          Source images now cropped to 515×773 (bottom 20% trimmed), so
          slider height ≈ 1.5 × width. Capped at 240px wide (≈ 360px tall)
          so the whole popup — title, paragraph, slider, "Got it" — fits an
          iPhone SE 375×667 viewport without internal scroll. */}
      <div style={{ maxWidth: 240, margin: "0 auto" }}>
        <RetouchTabbedSlider />
      </div>
      {/* Spacer between slider and Got it. Shrunk from 12 → 8 (2026-09-03)
          after the slider caption was removed — the button now sits closer
        ...
        >
          ...
          >
            <span>
              ...
              {deluxeCount > 0 && (
                <>
                  {deluxeCount} Glow Up Deluxe × $17.99
                </>
              )}
            </span>
...
type WildCardShot = {
  ...
  // this session; on shots restored from a resume link (2026-09-03) these are
  // populated from the server payload OR reconstructed by inferWildCardConfig
  // below when the record was saved before those fields existed.
  style?: string;
  lighting?: string;
...
// Label → regen-config lookup for wild cards restored from a resume link
// whose record predates the 2026-09-03 metadata add. The four supported
// labels are the only prefixes ever emitted by handleGenerateWildCards
// (women get creative-golden + executive-dramatic; men + gender-unknown get
// urban-studio + executive-dramatic). A label match reconstructs the exact
// (style, lighting, variationIndex) triple the shot was originally made
// with, so the ↻ button can fire /api/generate correctly. No match →
// returns undefined and the tile falls back to hiding the regen button.
const WILDCARD_LABEL_TO_CONFIG: {
  prefix: string;
  style: string;
  lighting: string;
  variationIndex: number;
}[] = [
  { prefix: "Creative Natural · Golden-hour light", style: "creative", lighting: "golden", variationIndex: 0 },
  { prefix: "Urban industrial · Studio light", style: "urban", lighting: "studio", variationIndex: 0 },
  { prefix: "Executive · Dramatic light", style: "executive", lighting: "dramatic", variationIndex: 3 },
];
function inferWildCardConfig(
  label: string,
): { style: string; lighting: string; variationIndex: number } | undefined {
  if (typeof label !== "string" || !label) return undefined;
  for (const c of WILDCARD_LABEL_TO_CONFIG) {
    if (label.startsWith(c.prefix)) {
      return { style: c.style, lighting: c.lighting, variationIndex: c.variationIndex };
    }
  }
  return undefined;
}
...
export default function App() {
  ...
  // session has the metadata it needs to fire a regen for a wild card. Older
  // records saved without these fields still work — the client's resume-path
  // label-parser reconstructs them from the wildcard's label prefix (see
  // WILDCARD_LABEL_TO_CONFIG below).
  const pendingWildCardsRef = useRef<
    {
      image: string;
      label: string;
      style?: string;
  ...
  // 6-image batch is still generating. Each headshot is a long (2–3 min)
  // request held open on the client; leaving mid-flight aborts all six and
  // they see nothing even though the server generated the images. A native
  // beforeunload prompt is the only thing that reliably interrupts an
  // accidental close/reload/back. Scoped tightly to the active generation
  // window so it never nags outside it. (2026-08-01)
  useEffect(() => {
    const generating =
      screen === "loading" &&
      !generationError &&
      readyCount < TOTAL_HEADSHOTS;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // 2026-09-03: also restore style/lighting/variationIndex so the ↻
        // regen button on each wild card tile is visible + fireable. Records
        // saved before this field existed won't have them — inferWildCardConfig
        // reconstructs from the label prefix ("Executive · Dramatic light · …",
        // "Urban industrial · Studio light · …", etc.) so legacy sessions
        // like chaeli's regain regen ability without a data backfill.
  ...
  // Preload the retouch slider images (Realistic + Polished + Glam) the moment
  // generation starts AND track when the FIRST one has fully loaded, so the
  // "Realism first" popup only appears once the slider is ready — text and
  // image together, never the image popping in several seconds late. During
  // generation the browser is busy with uploads + 6 parallel calls, which can
  // stall a plain preload, so we gate the popup on the actual load event below.
  // Per Kristi 2026-08-03. 2026-09-03: switched from the single retouch-versions
  // composite to the 3 per-tab images now that the popup uses the interactive
  // tabbed slider — preloading all three so tab switches inside the popup are
  // instant (no partial image flash on Realistic-vs-Polished tap).
  const [retouchImgReady, setRetouchImgReady] = useState(false);
  useEffect(() => {
    if (screen !== "loading") return;
    const sources = [
      "/marketing/retouch-realistic.jpg",
      "/marketing/retouch-glam.jpg",
      "/marketing/retouch-polished.jpg",
    ];
    let firstDone = false;
    const markFirst = () => {
      if (firstDone) return;
      firstDone = true;
      setRetouchImgReady(true);
    };
    for (const src of sources) {
      const img = new Image();
      img.onload = markFirst;
      img.onerror = markFirst; // a failed/slow image must never block the popup
      img.src = src;
      if (img.complete) markFirst(); // already cached (e.g. a second batch)
    }
  }, [screen]);
  ...
  // get auto-upsold to $17.99/photo.
  ...
  const handleRegenerateWildCard = async (index: number) => {
    ...
    try {
      ...
      setWildCards((prev) => {
        ...
        // reflects the regenerated wild card. Include the regen metadata
        // (style / lighting / variationIndex) so a resumed session can regen
        // it again without the label-parser fallback.
        pendingWildCardsRef.current = next
          ...
          .map((w) => ({
            ...
            style: w.style,
            lighting: w.lighting,
            variationIndex: w.variationIndex,
          }));
  ...
  const handleGenerate = async (
    ...
    if (successCount === 0) {
      ...
      // A zero-success batch is almost always a dropped client connection
      // (tab closed / backgrounded / navigated away mid-generation), NOT a
      // server or photo problem — the runtime logs show images generating
      // fine even when the customer sees this. So (a) refund the batch we
      // optimistically counted at the top of handleGenerate, so a dropped
      // connection never burns one of the customer's free generations and a
  ...
  const runWildCards = async (
    ...
    // the "your headshots are ready" email link (best-effort). Include the
    // regen metadata (style / lighting / variationIndex) so the ↻ button on
    // a resumed session can fire /api/generate with the right combo without
    // the label-parser fallback.
    ...
    <div style={{ minHeight: "100vh", background: C.pageBg, ...font }}>
      ...
      {screen === "loading" && (
        <LoadingScreen
          ...
          onRetry={() => {
            // Re-fire the same batch in place after a connection drop. The
            // batch was refunded on all-fail, so this isn't paywalled, and
            // reference photos + selections are still in state. Fall back to
            // the style screen only if we somehow lost the last selections.
            setGenerationError(null);
            setReadyCount(0);
            setGeneratedImages([]);
            if (lastSelections) {
              void handleGenerate(lastSelections);
            } else {
              setScreen("style");
            }
          }}
